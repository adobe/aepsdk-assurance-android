 ...
 * server over {@link AssuranceWebViewSocket}. Additionally, allows components controlling this class
 * to pause and resume processing (event forwarding). Note that the queue processing is paused by default
 * until controlling component invokes {@link #unblock()}.
 */
class OutboundEventQueueWorker extends EventQueueWorker<AssuranceEvent> {
	...
	/**
	 * Maximum number of bytes beyond which the {@code OutboundEventQueueWorker} fails to send data over
	 * the socket.
	 *
	 * Note that the data sent over {@link AssuranceWebViewSocket} is Base64 encoded. So the limit for an
	 * event in Base64 terms is (UTF8_EVENT_SIZE_BYTES * 3)/4
	 */
	@VisibleForTesting
	static final int MAX_EVENT_SIZE = (int) Math.floor((AssuranceWebViewSocket.MAX_DATA_LENGTH * 3) / 4.0);
	 ...
	 * <ol>
	 *     <li> allot a default ceiling size of 2KB for the metadata associated with the {@code AssuranceEvent}<li/>
	 *     <li> subtract above from {@code AssuranceWebViewSocket.MAX_DATA_LENGTH} that can be
	 *          transported by the {@link AssuranceWebViewSocket}<li/>
	 *     <li> reduce the remaining by half to accommodate JSON escaping with reconstruction <li/>
	 *     <li> Since the data sent over {@code AssuranceWebViewSocket} is Base64 encoded,
	 *          the limit for an event in Base64 terms is (UTF8_PAYLOAD_SIZE_BYTES) * (3/4) <li/>
	 * <ol/>
	 */
	@VisibleForTesting
	...
	OutboundEventQueueWorker(final ExecutorService executorService,
							 final AssuranceWebViewSocket socket,
							 final AssuranceClientInfo clientInfo) {
		this(executorService, socket, clientInfo, new LinkedBlockingQueue<AssuranceEvent>(),
	...
	protected boolean canWork() {
		// Ensure that the events are only attempted if the OutboundEventQueueWorker is unblocked and
		// the socket is connected.
		return canStartForwarding
			   && !(socket == null || socket.getState() != AssuranceWebViewSocket.SocketReadyState.OPEN);
	}
	 ...
	 * Invocation will be a no-op if the worker is already unblocked to prevent unnecessary
	 * client info events being sent.
	 *
	 * <p>
	 * This is the first event sent after a successful socket connection.
	 * This {@link AssuranceEvent} is not queued.
	 * ClientInfoEvent contains three major keys :
	 * <ul>
	 *     <li>Version : Representing the version of the Assurance SDK</li>
	 *     <li>AppSettings : A json representing AndroidManifest file</li>
	 *     <li>DeviceInfo : A map representing device information and current device state</li>
	 * </ul>
	 * </p>
	 */
	void sendClientInfoEvent() {
		...
		final AssuranceEvent clientInfoEvent = new AssuranceEvent(AssuranceConstants.AssuranceEventType.CLIENT,
				clientInfo.getData());
	...
	private void sendEventToSocket(final AssuranceEvent event) {
		...
		try {
			final byte[] eventData = event.getJSONRepresentation().getBytes(Charset.forName("UTF-8"));
			...
			if (eventData.length < MAX_EVENT_SIZE) {
				...
				if (event.getPayload() == null) {
					...
					Log.warning(Assurance.LOG_TAG, LOG_TAG, "Cannot send eventId: %s that exceeds permitted limit" +
								"but has an empty payload!", event.eventID);
					return;
				}
				...
				for (final AssuranceEvent chunkedEvent : chunkedEvents) {
					socket.sendData(chunkedEvent.getJSONRepresentation().getBytes(Charset.forName("UTF-8")));
				}
			}
			...
			Log.error(Assurance.LOG_TAG, LOG_TAG,
					  String.format("UnsupportedCharsetException while converting Assurance event object to bytes representation: %s",
	...
	static class OutboundEventChunker implements EventChunker<AssuranceEvent, AssuranceEvent> {
		...
		/**
		 * Converts {@param AssuranceEvent} into {@code AssuranceEvent}'s with payloads below
		 * {@code maxChunkSize}
		 *
		 * @param event AssuranceEvent that needs to be chunked.
		 * @return an empty List if the {@param event} is null;
		 *         singleton List comprising of {@param event} if payload is null;
		 * 		   singleton List comprising of {@param event} if content is within {@code maxChunkSize};
		 * 		   {@code List<AssuranceEvent>} resulting after chunking otherwise.
		 */
		@Override
		public List<AssuranceEvent> chunk(final AssuranceEvent event) {
			...
			if (event.getPayload() == null) {
				Log.warning(Assurance.LOG_TAG, LOG_TAG, "Cannot chunk event: %s with an empty payload!", event.eventID);
			...
			final ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(payloadBytes);
			...
			try {
				...
				while (byteArrayInputStream.read(buffer) != -1) {
					...
					payload.put(AssuranceConstants.AssuranceEventKeys.CHUNK_DATA, new String(buffer, Charset.forName("UTF-8")));

					final HashMap<String, Object> metadata = new HashMap<>();
					metadata.put(AssuranceConstants.AssuranceEventKeys.CHUNK_ID, chunkId);
					metadata.put(AssuranceConstants.AssuranceEventKeys.CHUNK_TOTAL, (int)totalChunks);
					metadata.put(AssuranceConstants.AssuranceEventKeys.CHUNK_SEQUENCE_NUMBER, chunkNumber++);

					final AssuranceEvent assuranceEvent = new AssuranceEvent(event.vendor,
							event.type, metadata, payload, event.timestamp);
					chunkedEvents.add(assuranceEvent);
				}
			} catch (final IOException e) {
				Log.warning(Assurance.LOG_TAG, LOG_TAG, "Failed to chunk event with ID: %s. Exception: %s",
