...
class AssurancePluginFakeEventGenerator implements AssurancePlugin {
	...
	public void onEventReceived(final AssuranceEvent event) {
		...
		if (fakeEventDetails == null || fakeEventDetails.isEmpty()) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG, "empty details obtained, Ignoring to generate fake event to eventHub");
			return;
		}

		if (!(fakeEventDetails.get("eventName") instanceof String)) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG, "Event name is invalid, Ignoring to generate fake event to eventHub");
			return;
		}

		if (!(fakeEventDetails.get("eventType") instanceof String)) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG, "Event type is invalid, Ignoring to generate fake event to eventHub");
			return;
		}
		...
		final Event fakeEvent = new Event.Builder((String)fakeEventDetails.get("eventName"),
				(String)fakeEventDetails.get("eventType"),
				(String)fakeEventDetails.get("eventSource")).setEventData(eventData).build();
		MobileCore.dispatchEvent(fakeEvent);
	}

	@Override
	public void onRegistered(final AssuranceSession parentSession) { /* no-op */ }

	@Override
	public void onSessionConnected() { /* no-op */ }

	@Override
	public void onSessionDisconnected(final int code) { /* no-op */ }

	@Override
	public void onSessionTerminated() { /* no-op */ }
}
