...
public final class AssuranceExtension extends Extension {
	...
	private static boolean shouldUnregisterOnTimeout = true; //keep the unregister timer active on launch
	 ...
	 * Constructor.
	 * <p>
	 * This method is called by MobileCore during the registration of Assurance extension with core.
	 * All the Assurance Extension listeners are registered.
	 * Thread : Background thread created by MobileCore
	 *
	 * @param extensionApi an instance of {@link ExtensionApi} with set of API's to interact with {@code MobileCore}.
	 */
	AssuranceExtension(final ExtensionApi extensionApi) {
		super(extensionApi);

		assuranceStateManager = new AssuranceStateManager(extensionApi, MobileCore.getApplication());
		assuranceConnectionDataStore = new AssuranceConnectionDataStore(MobileCore.getApplication());

		final List<AssurancePlugin> plugins = Collections.unmodifiableList(Arrays.asList(
		...
		assuranceSessionOrchestrator = new AssuranceSessionOrchestrator(MobileCore.getApplication(),
				assuranceStateManager, plugins, assuranceConnectionDataStore);
	}
	...
	void startSession(final String deeplink) {
		...
		if (assuranceSessionOrchestrator == null) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG,
						"Unable to start Assurance session. Make sure Assurance.registerExtension()" +
						"is called before starting the session. For more details refer " +
						"to https://aep-sdks.gitbook.io/docs/foundation-extensions/adobe-experience-platform-assurance#register-aepassurance-with-mobile-core");
			return;
		}

		if (assuranceSessionOrchestrator.getActiveSession() != null) {
			Log.debug(Assurance.LOG_TAG, LOG_TAG, "Unable to start Assurance session. Session already exists");
			return;
		}
		...
		if (StringUtils.isNullOrEmpty(sessionId)) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG,
						String.format("Unable to start Assurance session. The assurance sessionId obtained deeplink is invalid. Deeplink : %s",
									  deeplink));
			return;
		}

		final AssuranceEnvironment environment = AssuranceUtil.getEnvironmentFromQueryValue(uri.getQueryParameter(
					AssuranceConstants.DeeplinkURLKeys.START_URL_QUERY_KEY_ENVIRONMENT));

		// Create a new session. Note that new session creation via new deeplink will never have a PIN and
		...
		Log.trace(Assurance.LOG_TAG, LOG_TAG,
				  "Received sessionID. Initializing Assurance session. %s", sessionId);
	}
	...
	protected void onRegistered() {
		super.onRegistered();
		getApi().registerEventListener(EventType.WILDCARD, EventSource.WILDCARD, this::handleWildcardEvent);
		getApi().registerEventListener(EventType.ASSURANCE, EventSource.REQUEST_CONTENT, this::handleAssuranceRequestContent);
		getApi().registerEventListener(EventType.PLACES, EventSource.REQUEST_CONTENT,
									   new AssuranceListenerHubPlacesRequests(this));
		getApi().registerEventListener(EventType.PLACES, EventSource.RESPONSE_CONTENT,
		...
		// if assurance was not already connected, start timer to shutdown assurance, if startSession API is not called within 5 seconds
		...
		Log.debug(Assurance.LOG_TAG, LOG_TAG,
				  String.format("Assurance extension version %s is successfully registered",
	...
	/**
	 * Processes events trapped by AssuranceListenerHubWildcard and converts them into Assurance Events
	 *
	...
	void handleWildcardEvent(final Event event) {
		...
		payload.put(GenericEventPayloadKey.ACP_EXTENSION_EVENT_SOURCE, event.getSource().toLowerCase());
		payload.put(GenericEventPayloadKey.ACP_EXTENSION_EVENT_UNIQUE_IDENTIFIER, event.getUniqueIdentifier());
		...
		final AssuranceEvent assuranceEvent = new AssuranceEvent(AssuranceConstants.AssuranceEventType.GENERIC, payload);
		assuranceSessionOrchestrator.queueEvent(assuranceEvent);
	}

	void handleAssuranceRequestContent(Event event) {
		String sessionURL = DataReader.optString(event.getEventData(), AssuranceConstants.SDKEventDataKey.START_SESSION_URL,
							"");

		if ("".equals(sessionURL)) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG,
						"Unable to process start session event. could find start session URL in the event");
			return;
		}
	 ...
	 *
	 * @param event V5 Event object containing the shared state change event that triggered this update.
	 */
	private void processSharedStateEvent(final Event event, Map<String, Object> payload) {
		...
		try {
			final String stateOwner = DataReader.getString(eventData, AssuranceConstants.SDKEventDataKey.STATE_OWNER);
			...
			payload.put(AssuranceConstants.PayloadDataKeys.METADATA, new HashMap<String, Object>() {
				{
			...
			assuranceSessionOrchestrator.queueEvent(new AssuranceEvent(AssuranceConstants.AssuranceEventType.GENERIC, payload));
		} catch (final DataReaderException ex) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG,
						"Unable to extract state owner from shared state change event: " + ex.getLocalizedMessage());
		}
	}


	/**
	 * Call this methods to Unregsiter Assurance extension with the {@link MobileCore}
	 * This method clears the already queued events waiting to be sent on successful session connect.
	 * The shared state of Assurance extension is purged.
	 */
	private void shutDownAssurance() {
		Log.debug(Assurance.LOG_TAG, LOG_TAG,
				  "Timeout - Assurance did not receive deeplink to start Assurance session within 5 seconds. Shutting down Assurance extension");
	 ...
	 *
	 * @return {@code boolean} indicating if a there exists a saved session URL with which a connection can
	 * 	       be attempted.
	 */
