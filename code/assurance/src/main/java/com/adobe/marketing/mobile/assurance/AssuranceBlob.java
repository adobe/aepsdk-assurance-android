...
class AssuranceBlob {
	...
	/**
	 * Sends a binary blob of data to Project Assurance server to be recorded as an 'asset' for the current session.
	 * <p>
	 * Posts the binary blob to Project Assurance with the given contentType.  If no contentType is provided,
	 ...
	 *  <ol> blobID - NSString containing the asset ID of the newly stored asset </ol>
	 *  <ol> error - NSError representing the error that occurred </ol>
	 *
	 * @param blobData byte array containing the data to transmit
	 * @param contentType {@link String} containing the MIME type of the blob. Null, will default to application/octet-stream
	 * @param contentType the active {@link AssuranceSession} to which the blob is uploaded
	 * @param callback callback to be executed when the upload has completed (either successfully or with an error condition)
	 */
	static void upload(final byte[] blobData, final String contentType, final AssuranceSession session,
		...
		if (session == null) {
			uploadFailure(callback, "Unable to upload blob, assurance session instance unavailable");
			uploadFailure(callback, "Unable to upload blob, assurance session instance unavailable");
			return;
		}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					final String environmentFormat = AssuranceUtil.getURLFormatForEnvironment(session.getAssuranceEnvironment());
					final String sessionId = session.getSessionId();
					final String endpoint = String.format(UPLOAD_ENDPOINT_FORMAT, environmentFormat);
					final Uri uri = new Uri.Builder()
					.encodedPath(endpoint)
					.appendPath(UPLOAD_PATH_API)
					.appendPath(UPLOAD_PATH_FILEUPLOAD)
					.appendQueryParameter(UPLOAD_QUERY_KEY, (sessionId != null && !sessionId.isEmpty()) ? sessionId : "")
					.build();
					...
					connection.setRequestProperty(UPLOAD_HEADER_KEY_CONTENT_TYPE, "application/octet-stream");
					connection.setRequestProperty(UPLOAD_HEADER_KEY_FILE_CONTENT_TYPE, contentType);
					connection.setRequestProperty(UPLOAD_HEADER_KEY_CONTENT_LENGTH, "" + blobData.length);
					connection.setRequestProperty(UPLOAD_HEADER_KEY_ACCEPT, "application/json");
					...
					if (jsonResponse.has(RESPONSE_KEY_ERROR)) {
						final String error = jsonResponse.getString(RESPONSE_KEY_ERROR);
					...
					if (jsonResponse.has(RESPONSE_KEY_BLOB_ID)) {
						final String value = jsonResponse.getString(RESPONSE_KEY_BLOB_ID);

						if (value.isEmpty()) {
							uploadFailure(callback, "Uploading Blob failed, Invalid BlobId returned from the fileStorage server");
							return;
						}
					...
					uploadFailure(callback, "Uploading Blob failed, Json exception while parsing response, Error - " + ex);

				} catch (final MalformedURLException ex) {
					uploadFailure(callback, String.format("Uploading Blob failed, MalformedURLException %s", ex));
				} catch (final IOException ex) {
					uploadFailure(callback, String.format("Uploading Blob failed, IOException %s", ex));
				} catch (final Exception ex) {
					uploadFailure(callback, String.format("Uploading Blob failed with Exception : %s", ex));
				}
			}
		}).start();
	}
