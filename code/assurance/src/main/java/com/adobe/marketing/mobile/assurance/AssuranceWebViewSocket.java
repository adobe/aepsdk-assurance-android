...
final class AssuranceWebViewSocket {
	public enum SocketReadyState {
		CONNECTING, OPEN, CLOSING, CLOSED, UNKNOWN
	}
	...
	void connect(final String url) {
		if (!AssuranceUtil.isSafe(url)) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG, "URL is malformed, will not attempt to connect.");
			return;
		}
	...
	void sendData(final byte[] data) {
		...
		if (encodedData.length() > MAX_DATA_LENGTH) {
			Log.warning(Assurance.LOG_TAG, LOG_TAG,
						"Unable to send data packet, payload was " + encodedData.length() + " bytes, maximum is " +
						MAX_DATA_LENGTH + ".");
			return;
		}
	...
	/**
	 * Run the provided javascript in the webView.
	 * Uses mainThreadJoinSemaphore to execute javascript on socket thread one at a time.
	 *
	...
	private void runJavascript(final String jsString) {
		runOnSocketThread(new Runnable() {
			@Override
			public void run() {
				try {
					if (webView == null) {
					...
					// acquire this socket thread until javascript execution is completed in the mainthread. This ensures the javascript execution of websocket happens one by one.
					// mainThreadJoinSemaphore is initialized with permit 1
					mainThreadJoinSemaphore.acquire();
				} catch (final InterruptedException ex) {
					Log.error(Assurance.LOG_TAG, LOG_TAG, String.format("Socket unable to wait for JS semaphore: %s",
				...
				runOnMainThread(new Runnable() {
					...
					public void run() {
						if (webView != null) {
							...
							Log.error(Assurance.LOG_TAG, LOG_TAG, "WebView is null, unable to execute JS for socket communication.");
						}
	...
	/**
	 * Initializes the webView that runs socket connection.
	 * Thread : MainThread
	 */
		...
		runOnMainThread(new Runnable() {
			...
			public void run() {
				try {
					...
					if (cl == null) {
						Log.error(Assurance.LOG_TAG, LOG_TAG, "Socket unable to get class loader.");
						return;
					}

					currentSocket.webView = webView == null ? new WebView(MobileCore.getApplication()) : webView;
					...
					currentSocket.webView.setWebChromeClient(new WebChromeClient() {
						...
						public boolean onConsoleMessage(ConsoleMessage consoleMessage) {
							if (consoleMessage.messageLevel() == ConsoleMessage.MessageLevel.ERROR) {
								Log.error(Assurance.LOG_TAG, LOG_TAG, consoleMessage.message());
							}

							return super.onConsoleMessage(consoleMessage);
						}
					});
					currentSocket.webView.addJavascriptInterface(new WebViewJavascriptInterface(currentSocket), "nativeCode");
					currentSocket.webView.loadUrl(WEBSOCKET_HTML_PATH);
				} catch (final Exception ex) {
					Log.error(Assurance.LOG_TAG, LOG_TAG,
							  "Unexpected exception while initializing webview: " + ex.getLocalizedMessage());
				}
			}
		});
	}
	...
	private final class WebViewJavascriptInterface {
		...
		@JavascriptInterface
		public void onSocketClosed(final String reason, final short closeCode, final boolean wasClean) {
