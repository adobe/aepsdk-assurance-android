...
final class AssuranceConstants {
	...
	static final class SocketCloseCode {
		/**
		 * Represents an the result of a successful intentional
		 * closure.
		 * Example: the user clicking the disconnect button in the UI.
		 */
		...
		/**
		 * Represents a client error during socket connection resulting
		 * due the socket connection specifying an incorrect organization identifier.
		 */
		static final int ORG_MISMATCH = 4900;

		/**
		 * Represents a client error during socket connection
		 * Example: If clientInfoEvent is not the first event to socket
		 * or, if there are any missing parameters in the socket URL.
		 */
		static final int CLIENT_ERROR = 4400;

		/**
		 * Represents an error resulting due to the number of connections
		 * per session exceeding the default value - 200.
		 */
		static final int CONNECTION_LIMIT  = 4901;

		/**
		 * Represents an error resulting due to the number of Assurance events
		 * sent per minute. The default limit is 10k events per minute.
		 */
		static final int EVENT_LIMIT = 4902;

		/**
		 * Represents an error resulting due to the client attempting to connect
		 * to a session that has been deleted (session may have been deleted manually,
		 * or due to exceeding the 30 day limit)
		 */
	...
	enum AssuranceSocketError {
		GENERIC_ERROR("Connection Error", "The connection may be failing due to a network issue or an incorrect PIN. " +
					  "Please verify internet connectivity or the PIN and try again."),
		NO_ORGID("Invalid Configuration", "The Experience Cloud organization identifier is unavailable from the SDK. " +
				 "Ensure SDK configuration is setup correctly. See documentation for more detail."),
		ORGID_MISMATCH("Unauthorized Access", "The Experience Cloud organization identifier does not match with that of the Assurance session. " +
					   "Ensure the right Experience Cloud organization is being used. See documentation for more detail."),
		CONNECTION_LIMIT("Connection Limit Reached", "You have reached the maximum number of connected devices allowed for a session. " +
	...
	enum AssuranceEnvironment {
		 ...
		 * <p>
		 * Returns null if the provided string is not a valid {@link AssuranceEnvironment} enum value
		 *
		 ...
		 * <p>
		 * Defaults to PROD if the provided string is not a valid {@link AssuranceEnvironment} enum value
		 *
		 * @return {@link AssuranceEnvironment} value for provided status string
		 */
