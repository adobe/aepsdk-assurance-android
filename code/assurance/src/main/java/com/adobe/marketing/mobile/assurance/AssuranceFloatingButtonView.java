...
class AssuranceFloatingButtonView extends Button implements View.OnTouchListener {
	...
	public void setGraphic(final Graphic graphic) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
			this.setBackground(graphic == Graphic.CONNECTED ? ContextCompat.getDrawable(this.getContext(),
							   R.drawable.ic_assurance_active) : ContextCompat.getDrawable(this.getContext(), R.drawable.ic_assurance_inactive));
		}
	}
	...
	public boolean onTouch(View v, MotionEvent me) {
		if (me.getAction() == MotionEvent.ACTION_UP) {
			if (maxButtonTouch < BUTTON_MOVEMENT_TOLERANCE) { // perform click if up>down action didn't move button.
				performClick();
			}
