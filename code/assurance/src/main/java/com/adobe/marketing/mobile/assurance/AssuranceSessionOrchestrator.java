...
class AssuranceSessionOrchestrator {
	...
	/**
	 * Acts as a glue between the session presentation layer and session control layer.
	 * Responsible for relaying the UI operations relating to session management from the user back
	 * to the orchestrator.
	 */
	private final SessionUIOperationHandler sessionUIOperationHandler = new SessionUIOperationHandler() {
		@Override
		public void onConnect(final String pin) {
			if (session == null) {
				Log.error(Assurance.LOG_TAG, LOG_TAG, "PIN Confirmation without active session!");
				return;
			}

			if (StringUtils.isNullOrEmpty(pin)) {
				Log.error(Assurance.LOG_TAG, LOG_TAG, "Null/Empty PIN recorded. Cannot connect to session.");
				terminateSession();
				return;
			}
		...
		public void onDisconnect() {
			Log.debug(Assurance.LOG_TAG, LOG_TAG, "On Disconnect clicked. Disconnecting session.");
			terminateSession();
		}

		@Override
		public void onCancel() {
			Log.debug(Assurance.LOG_TAG, LOG_TAG, "On Cancel Clicked. Disconnecting session.");
			terminateSession();
		}
	};
	 ...
	 */
	private final AssuranceSession.AssuranceSessionStatusListener sessionStatusListener = new
	AssuranceSession.AssuranceSessionStatusListener() {
		@Override
		...
		public void onSessionTerminated() {
			// In case of a user initiated AssuranceSessionOrchestrator#terminateSession() will unregister the listener against the session
			// before disconnecting it. So this callback is never invoked in that flow.
			// However, in case of a disconnection initiated by the service, we need to cleanup references to the
			// active session to release resources.
			terminateSession();
		}
	};

	AssuranceSessionOrchestrator(final Application application,
		...
		this(application, assuranceStateManager, plugins, connectionURLStore, new ApplicationHandle(application,
	...
	AssuranceSessionOrchestrator(final Application application,
		...
		this.activityLifecycleObserver = new HostAppActivityLifecycleObserver(applicationHandle, this);
	...
	/**
	 * Creates a new {@code AssuranceSession} , registers the {@code sessionStatusListener} against it
	 * and also shares the shared state for the extension.
	 *
	 * @param sessionId the session id that the {@code AssuranceSession} should be connected to.
	 * @param environment the {@code AssuranceEnvironment} the {@code AssuranceSession} should be connected to.
	 * @param code the PIN code with which the {@code AssuranceSession} should be authenticated with.
	 */
	synchronized void createSession(final String sessionId,
									final AssuranceConstants.AssuranceEnvironment environment,
									final String code) {
		if (session != null) {
			Log.error(Assurance.LOG_TAG, LOG_TAG, "An active session already exists. Cannot create a new one.");
			return;
		}

		// create a new session
		session = sessionCreator.create(sessionId, environment, sessionUIOperationHandler, assuranceStateManager,
										plugins, connectionURLStore, applicationHandle, outboundEventBuffer);
	...
	synchronized void terminateSession() {
		Log.debug(Assurance.LOG_TAG, LOG_TAG, "Terminating active session. Clearing the queued" +
	...
	/**
	 * Attempt to reconnect to a previously connected session that has not been explicitly disconnected
	 * by the user via a valid connection url.
	 * @return true if there exists a valid connection url that can be reconnected to,
	 *         false if a valist connection url does not exist.
	 */
	boolean reconnectToStoredSession() {
		final String connectionURL  =  connectionURLStore.getStoredConnectionURL();
		Log.debug(Assurance.LOG_TAG, LOG_TAG, "Attempting to reconnect to stored URL: " + connectionURL);
		...
		final AssuranceConstants.AssuranceEnvironment environment = AssuranceUtil.getEnvironmentFromQueryValue(
					uri.getQueryParameter(
						AssuranceConstants.DeeplinkURLKeys.START_URL_QUERY_KEY_ENVIRONMENT));
		...
		Log.trace(Assurance.LOG_TAG, LOG_TAG,
				  "Initializing Assurance session. %s using stored connection details:%s ", sessionId, connectionURL);
	 ...
	 * @return true if extension is waiting for the first session to be established on launch (before shutting down)
	...
	/**
	 * Interface that acts as a glue between the session presentation layer and session control layer.
	 */
	...
	/**
	 * Serves as a handle to retrieve the context information about the host application that this extension
	 * is registered against. Intentionally a separate class than the  {@code HostAppActivityLifecycleObserver}
	 * to limit access to the publicly overridden methods of an activity lifecycle observer.
	 */
	static class ApplicationHandle {
		...
		ApplicationHandle(final Application application,
	 ...
	 * is registered against. Acts as the primary source of deeplink based session initiation.
	 * This class also relays the details of the current activity for use by other components
	 * of the extension via the {@code ApplicationHandle} and the active {@code AssuranceSession}
	 *
	 */
	static class HostAppActivityLifecycleObserver implements Application.ActivityLifecycleCallbacks {
		...
		HostAppActivityLifecycleObserver(final ApplicationHandle applicationHandle,
		...
		public void onActivityCreated(@NonNull Activity activity,
			...
			Log.trace(Assurance.LOG_TAG, LOG_TAG, "Session Activity Hook - onActivityCreated called " +
					  activity.getClass().getCanonicalName());
		}

		@Override
		public void onActivityStarted(@NonNull Activity activity) {
			Log.trace(Assurance.LOG_TAG, LOG_TAG, "Session Activity Hook - onActivityStarted called " +
					  activity.getClass().getCanonicalName());
		...
		public void onActivityResumed(@NonNull Activity activity) {
			Log.trace(Assurance.LOG_TAG, LOG_TAG, "Session Activity Hook - onActivityResumed called " +
					  activity.getClass().getCanonicalName());
		...
		public void onActivityPaused(@NonNull Activity activity) {
			Log.trace(Assurance.LOG_TAG, LOG_TAG, "Session Activity Hook - onActivityPaused called " +
					  activity.getClass().getCanonicalName());
		...
		public void onActivityStopped(@NonNull Activity activity) {
			Log.trace(Assurance.LOG_TAG, LOG_TAG, "Session Activity Hook - onActivityStopped called " +
					  activity.getClass().getCanonicalName());
		}
		...
		public void onActivityDestroyed(@NonNull Activity activity) {
			Log.trace(Assurance.LOG_TAG, LOG_TAG, "Session Activity Hook - onActivityDestroyed called " +
					  activity.getClass().getCanonicalName());
	...
	/**
	 * A convenience class for mocking session creation during tests.
	 * Exists ONLY for test convenience.
	 */
	@VisibleForTesting
	static class AssuranceSessionCreator {
		AssuranceSession create(final String sessionId,
								final AssuranceConstants.AssuranceEnvironment environment,
			...
			return new AssuranceSession(applicationHandle, assuranceStateManager, sessionId, environment,
										connectionURLStore, sessionUIOperationHandler, plugins, outboundEventBuffer);
		}
	}
}
