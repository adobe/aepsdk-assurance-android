...
final class AssuranceUtil {
	...
	private static final HashSet<String> VALID_CONNECTION_PARAMETER_NAMES = new HashSet<>(
			Arrays.asList(
	 ...
	 * <p>
	 * This method never returns a null value.
	 * This method return an empty string for prod assuranceEnvironment.
	 *
	 * @param assuranceEnvironment an  {@link AssuranceEnvironment} enum representing the current assuranceEnvironment of Assurance session.
	 ...
	 * <p>
	 * If the query value is null or empty, the environment value returned is {@link AssuranceEnvironment#PROD}.
	 * This is because the deeplink doesn't contain a query value for prod environment Assurance session.
	 *
	 * @param queryValue A {@link String} query value for environment obtained from deeplink URL
	 * @return {@link AssuranceEnvironment} value representing the environment of the Assurance session
	 */
	 ...
	 * <p>
	 * This method returns null in following occasions:
	 * 1.If the provided Uri is null
	 * 2.If the Uri doesn't contain sessionId value in predefined key
	 * 3.If the sessionId is not a valid UUID
	 *
	...
	static String getValidSessionIDFromUri(final Uri deeplinkUri) {
		...
		final String uuidSessionID = deeplinkUri.getQueryParameter(
	...
	/**
	 * Determines if the connection string provided is safe against JS injection attack.
	 * Note that this method does not validate existence or correctness of all the required URL
	 * query parameters. So a non existent parameter is considered safe.
	 *
	...
	/**
	 * Checks if there are any invalid parameters present in the connection {@code Uri}
	 * candidate provided. Any query parameter that does not exist in {@link AssuranceUtil#VALID_CONNECTION_PARAMETER_NAMES}
	 * is considered unwanted and therefore invalid.
	 *
