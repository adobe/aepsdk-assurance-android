...
final class AssuranceIOUtils {
	 ...
	 * <p>
	 * It opens a {@link XmlResourceParser} for @xmlFileName. Use the opened {@code XmlResourceParser} to reads XML tags and convert them to {@code JSONObject}.
	 * for ex: <manifest><application name="abc"/><manifest/> will convert to {"manifest: {application:{"name":"abc"}}"}
	 *
	 * Returns an empty {@code JSONObject} if there is an {@code Exception} thrown by {@code XmlResourceParser}.
	 * </p>
	...
	static JSONObject parseXMLResourceFileToJson(final String xmlFileName) {
		...
		try {
			...
			xmlResParser = appContext.createPackageContext(appContext.getPackageName(),
						   0).getAssets().openXmlResourceParser(xmlFileName);
			xmlResParser.next(); //Move the parser to XmlPullParser.START_DOCUMENT
			return convertXMLToJSON(xmlResParser);
		} catch (final Exception e) {
			Log.debug(Assurance.LOG_TAG, "Failed to create parse %s file. Error: %s", xmlFileName, e.getMessage());
		} finally {
	 ...
	 */
	static JSONObject convertXMLToJSON(final XmlPullParser xmlPullParser) throws IOException,
				...
				case XmlPullParser.END_TAG:
					...
					if (jsonObjectParent.has(xmlPullParser.getName())) {
						if (jsonObjectParent.get(xmlPullParser.getName()) instanceof JSONArray) {
							final JSONArray arr = (JSONArray) jsonObjectParent.get(xmlPullParser.getName());
							arr.put(jsonObjChild);
						} else {
							final JSONObject obj = (JSONObject) jsonObjectParent.get(xmlPullParser.getName());
