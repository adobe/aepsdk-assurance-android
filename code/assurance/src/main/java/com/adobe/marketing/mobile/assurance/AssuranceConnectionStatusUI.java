...
class AssuranceConnectionStatusUI implements AssuranceFullScreenTakeover.FullScreenTakeoverCallbacks {
	...
	AssuranceConnectionStatusUI(final AssuranceSessionOrchestrator.SessionUIOperationHandler uiOperationHandler,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					final InputStream is = cl.getResourceAsStream("assets/StatusInfo.html");

					if (is == null) {
						Log.error(Assurance.LOG_TAG, LOG_TAG, "Unable to open StatusInfo.html");
						return;
					}
					...
					thisRef.statusTakeover = new AssuranceFullScreenTakeover(applicationHandle.getAppContext(), html, thisRef);
	...
	void addUILog(final UILogColorVisibility visibility, final String message) {
		...
		if (currentTakeover != null && message != null && visibility != null) {
			final String cleanMessage = message.replace("\\", "\\\\")
										.replace("\"", "\\\"")
										.replace("\n", "<br>")
										.replace("\t", "&nbsp;&nbsp;&nbsp;&nbsp;");
			Log.trace(Assurance.LOG_TAG, LOG_TAG, "Client Side Logging (%s) - %s", visibility.getValue(), message);
			final String js = String.format(Locale.US, "addLog(%d, \"%s\");", visibility.getValue(), cleanMessage);
			this.statusTakeover.runJavascript(js);
		} else {
			Log.warning(Assurance.LOG_TAG, LOG_TAG, "Status update failed due to lack of statusTakeover reference");
		}
	}
	...
	public boolean onURLTriggered(final String url) {
		...
		if ("disconnect".equals(triggeredUrl.getHost())) {
			...
			Log.warning(Assurance.LOG_TAG, LOG_TAG,
						String.format("Unknown url coming from status takeover redirect: Url - %s",
									  url));
		}
