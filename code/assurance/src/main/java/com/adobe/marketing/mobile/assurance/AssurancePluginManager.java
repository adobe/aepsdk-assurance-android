...
class AssurancePluginManager {
	...
	void addPlugin(final AssurancePlugin plugin) {
		...
		final ConcurrentLinkedQueue<AssurancePlugin> existingQueue = plugins.putIfAbsent(vendorID, newVendorQueue);
	...
	/**
	 * Notifies registered plugins about {@link AssuranceEvent}'s received by the {@link AssuranceSession}
	 * @param event the {@link AssuranceEvent}'s received by {@link AssuranceSession}
	 */
	void onAssuranceEvent(final AssuranceEvent event) {
		final ConcurrentLinkedQueue<AssurancePlugin> pluginsForVendor = plugins.get(event.getVendor());

		if (pluginsForVendor == null) {
			Log.debug(Assurance.LOG_TAG, LOG_TAG, String.format("There are no plugins registered to handle incoming" +
					  " Assurance event with vendor : %s", event.getVendor()));
			return;
		}
		...
		for (final AssurancePlugin plugin : pluginsForVendor) {
			...
			// refrain from calling `onEventReceived` on a plugin if plugin's controlType is empty or #AssuranceConstants.ControlType.NONE
			if (pluginControlType == null || pluginControlType.isEmpty()
