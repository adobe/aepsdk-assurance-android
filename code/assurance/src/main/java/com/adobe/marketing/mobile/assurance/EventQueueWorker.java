...
/**
 * Provides a template for managing the processing of a queue of work items. Aims to separate
 * the lifecycle of the worker thread(s) processing work items with the queue that they are fetched from.
 * Allows sub-classes to be agnostic of the thread management.
 *
 * TODO: Core 2.0 now provides a {@link com.adobe.marketing.mobile.util.SerialWorkDispatcher} which
 * can be used to replace this class.
 */
abstract class EventQueueWorker<T> implements Runnable {
	private static final String LOG_TAG = "EventChunker";

	/**
	 * Interface contract for chunking an event of type T into
	 * events of type V.
	 *
	...
	/**
	 * Denotes the activeness of the {@link EventQueueWorker}. An {@link EventQueueWorker} is said to
	 * be active if {@link #start()} is invoked at-least once after {@link #stop()} (if ever invoked).
	 * Note that this is not the state of the worker-thread.
	 */
	...
	@VisibleForTesting
	EventQueueWorker(final ExecutorService executorService, final LinkedBlockingQueue<T> workQueue) {
		this.workQueue = workQueue;
		this.executorService = executorService;
	}

	/**
	 * Enqueues an item to the end of the {@link #workQueue}. Additionally,
	 * resumes the queue processing if the {@link EventQueueWorker} is active
	...
	/**
	 * Puts the {@link EventQueueWorker} in active state and starts processing the {@link #workQueue}
	...
	/**
	 * Invoked on the calling thread, immediately before processing the items in the queue for the first time.
	 * Implementers are expected to perform any one-time setup operations (bound by the activeness of
	 * {@link EventQueueWorker}) before processing starts.
	 */
	protected abstract void prepare();

	/**
	 * Invoked before processing each work item. Results in the worker thread being completed
	 * if the implementer returns false. Implementers are expected to enforce any conditions that needs
	 * to be checked before performing work here.
	 * This is invoked from the background worker thread that the {@link EventQueueWorker} maintains.
	 *
	...
	public void run() {
		while (!Thread.interrupted() && canWork() && workQueue.peek() != null) {
			try {
				...
				Log.error(Assurance.LOG_TAG, LOG_TAG, "Background worker thread(InboundEventWorker) interrupted: "
						  + exception.getLocalizedMessage());
				// https://docs.oracle.com/javase/7/docs/technotes/guides/concurrency/threadPrimitiveDeprecation.html
				// read in answer for question : How do I stop a thread that waits for long periods (e.g., for input)?
		...
		Log.debug(Assurance.LOG_TAG, LOG_TAG, "No more items to process. Finishing current job : %s for %s",
				  Thread.currentThread().toString(), this.getClass().getSimpleName());
	}

	/**
	 * Perform processing on the workItem. This is invoked on a thread different from the rest of the
	...
	/**
	 * Puts the {@link EventQueueWorker} into inactive state and clears the {@link #workQueue}.
	 * The {@link EventQueueWorker} needs to be started again via {@link #start()} to do new work.
	 * Calling {@link #resume()} will have no affect on the sate of the {@link EventQueueWorker} after
