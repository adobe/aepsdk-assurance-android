	...
	public void hear(final Event event) {
		...
		if (eventName.equals(GET_NEARBY_PLACES_EVENT_NAME)) {
			...
			try {
				...
				final String logMessage = String.format(Locale.US, "Places - Requesting %d nearby POIs from (%.6f, %.6f)", count,
														latitude,
														longitude);
				parent.logLocalUI(UILogColorVisibility.NORMAL, logMessage);
			} catch (final DataReaderException ex) {
				Log.warning(Assurance.LOG_TAG, LOG_TAG, "Unable to log-local Places event: " + ex.getLocalizedMessage());
			}
