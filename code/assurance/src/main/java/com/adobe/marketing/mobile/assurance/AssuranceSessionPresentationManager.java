...
class AssuranceSessionPresentationManager {
	...
	AssuranceSessionPresentationManager(final AssuranceStateManager assuranceStateManager,
		...
		button = new AssuranceFloatingButton(applicationHandle, new View.OnClickListener() {
			@Override
		...
		urlProvider = new AssurancePinCodeEntryURLProvider(applicationHandle, uiOperationHandler, assuranceStateManager);
	...
	/**
	 * Shows the UI elements (currently connecting screen) that are required when a session connection
	...
	void onSessionDisconnected(final int closeCode) {
		switch (closeCode) {
			...
			default:
				displayError(AssuranceConstants.AssuranceSocketError.GENERIC_ERROR, AssuranceConstants.SocketCloseCode.ABNORMAL);
		}
	}

	/**
	 * Shows the UI elements that are required when a session is re-connecting (implicitly) after encountering
	 * an error.
	 */
	void onSessionReconnecting() {
		...
		logLocalUI(AssuranceConstants.UILogColorVisibility.HIGH,
	...
	void onSessionStateChange(final AssuranceWebViewSocket.SocketReadyState newState) {
		if (button != null) {
			button.setCurrentGraphic(newState == AssuranceWebViewSocket.SocketReadyState.OPEN ?
									 AssuranceFloatingButtonView.Graphic.CONNECTED : AssuranceFloatingButtonView.Graphic.DISCONNECTED);
		}
	}

	/**
	 * Acts on the notification about an activity of the host app starting. Needed to manage
	 * edge cases that arise from activity launch sequences.
	 *
	 * @param activity the activity of the host application that has started
	 */
	void onActivityStarted(final Activity activity) {
		// If the Assurance pinpad screen is displayed and the Application tries to start a new activity
		// then reorder the Assurance pinpad activity to display on top
		if (AssuranceFullScreenTakeoverActivity.isDisplayed
				&& !(FULLSCREEN_TAKEOVER_ACTIVITY_CLASSNAME.equals(activity.getClass().getSimpleName()))) {

			// bring activity to front
			final Intent fullscreen = new Intent(activity, AssuranceFullScreenTakeoverActivity.class);
	...
	/**
	 * Notification about an activity of the host app resuming. Needed to manage
	 * button placement on the current activity as well as launching a pin screen as necessary.
	 *
	...
	void onActivityResumed(final Activity activity) {
		...
		if (urlProvider != null) {
			...
			if (deferredRunnable != null) {
				Log.debug(Assurance.LOG_TAG, LOG_TAG, "Session Activity Hook - Deferred connection dialog found, triggering.");
	...
	/**
	 * Notification about an activity of the host app being destroyed. Needed to manage
	 * button destruction.
	 *
	...
	private void displayError(final AssuranceConstants.AssuranceSocketError socketError, final int closeCode) {
		if (urlProvider != null && urlProvider.isDisplayed()) {
			// If this is an unhandled/abnormal error while the PIN screen is on, set the retry flag to true.
			// Else the "Cancel" button on the PIN screen will allow socket disconnection and cleaning up of the UI Elements.
			urlProvider.onConnectionFailed(socketError, closeCode == AssuranceConstants.SocketCloseCode.ABNORMAL);
		} else {
			if (closeCode == AssuranceConstants.SocketCloseCode.ABNORMAL) {
				...
				// that all abnormal errors can be re-tried and hence a retry attempt will enforce UI changes.
				return;
			}
	...
	private void showErrorDisplay(final AssuranceConstants.AssuranceSocketError socketError) {
		...
		try {
			final Intent errorScreen = new Intent(currentActivity, AssuranceErrorDisplayActivity.class);
			errorScreen.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
			errorScreen.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
			errorScreen.putExtra(AssuranceConstants.IntentExtraKey.ERROR_NAME, socketError.getError());
			errorScreen.putExtra(AssuranceConstants.IntentExtraKey.ERROR_DESCRIPTION, socketError.getErrorDescription());
