...
class AssurancePluginScreenshot implements AssurancePlugin {
	...
	public void onEventReceived(final AssuranceEvent event) {
		...
		listener = new CaptureScreenShotListener() {
			@Override
	...
	private void getCurrentScreenShot(final CaptureScreenShotListener captureScreenShotListener) {
		if (parentSession == null) {
			Log.error(Assurance.LOG_TAG, LOG_TAG, "Unable to take screenshot, Assurance session instance unavailable.");
			return;
		}
	...
	private void sendScreenshot(final ByteArrayOutputStream baos) {
		if (parentSession == null) {
			Log.error(Assurance.LOG_TAG, LOG_TAG, "Unable to send screenshot, Assurance session instance unavailable");
			return;
		}

		AssuranceBlob.upload(baos.toByteArray(), "image/jpeg", parentSession, new AssuranceBlob.BlobUploadCallback() {
			@Override
			public void onSuccess(final String blobID) {
				...
				final AssuranceEvent screenshotEvent = new AssuranceEvent(AssuranceConstants.AssuranceEventType.BLOB, responsePayload);
				...
				if (parentSession != null) {
					...
					Log.warning(Assurance.LOG_TAG, LOG_TAG,
								"Assurance Session instance is null for AssurancePluginScreenshot, Cannot send the screenshot event.");
				}
			}
			...
			public void onFailure(final String reason) {
				...
				final AssuranceEvent screenshotFailEvent = new AssuranceEvent(AssuranceConstants.AssuranceEventType.BLOB,
						responsePayload);
