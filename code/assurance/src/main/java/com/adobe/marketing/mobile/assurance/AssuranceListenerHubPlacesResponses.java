...
class AssuranceListenerHubPlacesResponses implements ExtensionEventListener {
	...
	public void hear(final Event event) {
		...
		if (eventName.equals(NEARBY_PLACES_EVENT_NAME)) {
			final List<Map<?, ?>> nearbyPois = DataReader.optTypedList(Map.class, eventData, NEARBY_POI_KEY, new ArrayList());
			final StringBuilder sb = new StringBuilder();
			sb.append(String.format(Locale.US, "Places - Found %d nearby POIs%s", nearbyPois.size(),
									!nearbyPois.isEmpty() ? ":" : "."));
			...
			for (final Map<?, ?> poiMap : nearbyPois) {
				if (AssuranceUtil.isStringMap(poiMap)) {
					...
					if (region != null) {
						final boolean userIsInside = DataReader.optBoolean(poi, USER_WITHIN_KEY, false);
			...
			final Map<String, Object> triggeringRegion = DataReader.optTypedMap(Object.class, eventData, TRIGGERING_REGION_KEY,
					new HashMap());
			final String regionName = DataReader.optString(triggeringRegion, REGION_NAME_KEY, null);

			if (regionName != null) {
				final String regionEventType = DataReader.optString(eventData, REGION_EVENT_TYPE_KEY, "");
				final String logMessage = String.format(Locale.US, "Places - Processed %s for region \"%s\".",
														regionEventType,
