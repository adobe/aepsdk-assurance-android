 ...
 * event queue workers, plugins and the presentation elements associated with the session.
 * The lifecycle of this class is equivalent to the lifecycle of the socket connection maintained by
 * it.
 * An {@code AssuranceSession} is "valid" if an instance exists (either waiting for a connection
 * to be established, or one which is already connected).
 */
