...
class AssurancePluginLogForwarder implements AssurancePlugin {
	...
	private final class LogForwardThread implements Runnable {
		...
		public void run() {
			try {
				final Process procRemoveUnecessaryLogs = new ProcessBuilder().command("logcat", "-P", "").start();
				final String processIdCommand = String.format("--pid=%s", android.os.Process.myPid());
				final Process proc = new ProcessBuilder().command("logcat", processIdCommand, "-bmain", "-vlong").start();
				...
				while (logEnabled && !Thread.interrupted()) {
					try {
						...
						if (logLine != null && checkIfHeader(logLine)) {
							...
							final AssuranceEvent logEvent = new AssuranceEvent(AssuranceConstants.AssuranceEventType.LOG, eventPayload);
							final AssuranceSession session = parentSession.get();
						...
						Log.error(Assurance.LOG_TAG, LOG_TAG, String.format("Log forwarding error reading line: %s",
				...
				// handle exception
				Log.error(Assurance.LOG_TAG, LOG_TAG, String.format("Log forwarding error while sending logs: %s" +
						  ex.getLocalizedMessage()));
			}
