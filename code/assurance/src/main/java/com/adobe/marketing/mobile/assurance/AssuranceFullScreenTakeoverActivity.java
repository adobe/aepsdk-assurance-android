...
public class AssuranceFullScreenTakeoverActivity extends Activity {
	...
	protected static WeakReference<AssuranceFullScreenTakeover> weakFullScreenTakeoverReference = new WeakReference<>(null);
	...
	public void onResume() {
		...
		super.onResume();
		final AssuranceFullScreenTakeover fullScreenTakeover = weakFullScreenTakeoverReference.get();
		...
		// Always use a weak reference to context outside of its lifecycle. Not doing so will cause the context/activity to leak.
		...
		try {
			...
			if (rootViewGroup == null) {
				Log.error(Assurance.LOG_TAG, LOG_TAG, "Failed to show fullscreen takeover, could not get root view group.");
				dismiss();
			} else {
			...
			Log.error(Assurance.LOG_TAG, LOG_TAG, "Failed to show fullscreen takeover: %s", ex.getLocalizedMessage());
			dismiss();
		}
	}
