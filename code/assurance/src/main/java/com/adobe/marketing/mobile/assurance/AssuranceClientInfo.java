...
final class AssuranceClientInfo {
	 ...
	 *   <li> deviceInfo - Information about the state of the device, battery status, screensize, device name and manufacturer etc.
	 *   <li> appSettings - Applications Manifest.xml file parsed in JSON format
	 *   <li> type = connect, representing that this event is initializing a socket connection with server
	 * </ol>
	 *
	...
	final Map<String, Object> getData() {
		...
		// TODO : https://jira.corp.adobe.com/browse/AMSDK-10653 send Manifest xml using blob service
	 ...
	 * <ol>
	 *   <li> Canonical platform name - Android , representing canonical platform name for the device.
	 ...
	 * <p>
	 * Returns nil if unable to retrieve the carrier name.
	 *
	...
	private String getMobileCarrierName() {
		...
		final TelephonyManager telephonyManager = ((TelephonyManager) context.getSystemService(Application.TELEPHONY_SERVICE));
	...
	/**
	 * 	The current battery level for the device. This is integer value ranging from 1 to 100. If unable to fetch battery value -1 is returned
	 *
	...
	private int getBatteryPercentage() {
		...
		if (Build.VERSION.SDK_INT >= 21) {
			...
			final int level = batteryStatus != null ? batteryStatus.getIntExtra(BatteryManager.EXTRA_LEVEL, -1) : -1;
			final int scale = batteryStatus != null ? batteryStatus.getIntExtra(BatteryManager.EXTRA_SCALE, -1) : -1;
	...
	private Boolean isLocationEnabled() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
			...
			final int mode = Settings.Secure.getInt(context.getContentResolver(), Settings.Secure.LOCATION_MODE,
	 ...
	 * <ol>
	 *  <li> Denied : The user denied the use of location services for the app/ Or the user has not been prompted yet to use location permission
	 *  <li> When in use : The user authorized the app to start location services while it is in use.
	...
	private String getCurrentLocationPermission() {
		...
		if (context == null) {
			// Unable to check location permission, App context is not available. Defaulting acquired permission level to unknown
			return VALUE_UNKNOWN;
		}

		final int permissionStatus = ActivityCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION);
	...
	/**
	 * Helper method to know if Background location permission is provided by default to the application.
	 */
	...
	private boolean isBackgroundPermissionGranted() {
		...
		final int permissionState = ActivityCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION);
		...
		if (permissionState == PackageManager.PERMISSION_GRANTED) {
			...
			if (!isBackgroundLocationAccessGrantedByDefault()) {
				final int bgLocationPermissionState = ActivityCompat.checkSelfPermission(context,
	 ...
	 * The API to read PowerSaveMode was introduces in Android API version 21.
	 * For Android API versions before 21, this method returns false.
	 *
