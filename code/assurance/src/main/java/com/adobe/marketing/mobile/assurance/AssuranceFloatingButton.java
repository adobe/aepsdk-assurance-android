...
class AssuranceFloatingButton implements AssuranceSessionLifecycleListener {
	...
	AssuranceFloatingButton(final AssuranceSessionOrchestrator.ApplicationHandle applicationHandle,
	 ...
	 * <p>
	 * An instance of {@link AssuranceFloatingButtonView} needs to be already instantiated and managed by this manager before this method can be used to display
	...
	private void display(final float x, final float y, final Activity activity) {
		// Make sure we don't overlay a assurance ui view with the floating button... hilarity will ensue.
		if (activity instanceof AssuranceFullScreenTakeoverActivity) {
			Log.trace(Assurance.LOG_TAG, LOG_TAG, "Skipping FloatingButton Overlay due to Assurance view presentation.");
			return;
		}

		//We will use the absolute width and height later if the root view has not been measured by then.
		activity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				AssuranceFloatingButtonView floatingButtonView = rootViewGroup.findViewWithTag(AssuranceFloatingButtonView.VIEW_TAG);
				...
				if (floatingButtonView == null) {
					Log.error(Assurance.LOG_TAG, LOG_TAG, "Unable to create floating button for activity `%s`", activityClassName);
					return;
				}

				floatingButtonView.setGraphic(currentGraphic);
				floatingButtonView.setVisibility(buttonDisplayEnabled ? VISIBLE : GONE);
				floatingButtonView.setOnPositionChangedListener(new AssuranceFloatingButtonView.OnPositionChangedListener() {
					@Override
				...
				final AssuranceFloatingButtonView buttonViewForLayoutListener = floatingButtonView;
				ViewTreeObserver.OnGlobalLayoutListener onGlobalLayoutListener = new ViewTreeObserver.OnGlobalLayoutListener() {
					@Override
					public void onGlobalLayout() {
						removeOnGlobalLayoutListenerCompat(buttonViewForLayoutListener, this);

						if (x >= 0 && y >= 0) {
							//Adjust x and y to account for orientation change.
							lastKnownXPos = adjustXBounds(buttonViewForLayoutListener, width, x);
							lastKnownYPos = adjustYBounds(buttonViewForLayoutListener, height, y);
						} else {
							lastKnownXPos = width - buttonViewForLayoutListener.getWidth();
							lastKnownYPos = 0.0f;
						}

						buttonViewForLayoutListener.setPosition(lastKnownXPos, lastKnownYPos);
					}
				};
				...
				try {
					...
					Log.trace(Assurance.LOG_TAG, LOG_TAG, "Failed to add floating button view: Error - %s",
				...
				if (layoutParams != null) {
					layoutParams.width = layoutParams.height = Math.round(displayMetrics.density * BUTTON_SIZE);
	...
	private void removeFloatingButtonFromActivity(final Activity activity) {
		...
		if (activity == null) {
			Log.error(Assurance.LOG_TAG, LOG_TAG, "Cannot remove floating button, activity is null.");
			return;
		}
		...
		activity.runOnUiThread(new Runnable() {
			...
			public void run() {
				final ViewGroup rootViewGroup = (ViewGroup) activity.getWindow().getDecorView().getRootView();
				AssuranceFloatingButtonView floatingButton = rootViewGroup.findViewWithTag(AssuranceFloatingButtonView.VIEW_TAG);

				if (floatingButton != null) {
					// Always reset listeners assigned to the Views. Not doing so will cause a leak, especially
					// in this case where the listeners hold a reference to the context indirectly.
					floatingButton.setOnPositionChangedListener(null);
					floatingButton.setOnClickListener(null);
					floatingButton.setVisibility(GONE);
				} else {
					Log.debug(Assurance.LOG_TAG, LOG_TAG, "No floating button found for removal on activity `%s`",
							  activityClassName);
				}
			}
		});
	 ...
	 */
	private float adjustXBounds(final AssuranceFloatingButtonView floatingButtonView, final float screenWidth,
								final float oldX) {
		//        if (floatingButtonView != null && oldX > (screenWidth - floatingButtonView.getWidth())) {
	 ...
	 */
	private float adjustYBounds(final AssuranceFloatingButtonView floatingButtonView, final float screenHeight,
								final float oldY) {
	 ...
	 *
	 * @param floatingButtonView The {@link AssuranceFloatingButtonView} instance which is used to retrieve the {@link ViewTreeObserver}
	 * @param onGlobalLayoutListener The {@link ViewTreeObserver.OnGlobalLayoutListener} instance
	 */
	private void removeOnGlobalLayoutListenerCompat(AssuranceFloatingButtonView floatingButtonView,
	...
	private void manageButtonDisplayForActivity(final Activity activity) {
		if (activity == null) {
			Log.debug(Assurance.LOG_TAG, LOG_TAG, LOG_TAG, "[manageButtonDisplayForActivity] activity is null");
			return;
		}
		...
		if (!buttonDisplayEnabled) {
			...
			if (managedButtonViews.get(activityClassName) == null
					&& !AssuranceFullScreenTakeoverActivity.class.getSimpleName().equalsIgnoreCase(activity.getClass().getSimpleName())) {
				...
				final AssuranceFloatingButtonView newButton = new AssuranceFloatingButtonView(activity);
