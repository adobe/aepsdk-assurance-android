...
public class OutboundEventChunkerTest {
	...
	public void test_chunk_nullEvent() {
		final OutboundEventQueueWorker.OutboundEventChunker outboundEventChunker =
			new OutboundEventQueueWorker.OutboundEventChunker(OutboundEventQueueWorker.MAX_PAYLOAD_CHUNK_SIZE);
	...
	public void test_chunk_nullPayload() {
		final OutboundEventQueueWorker.OutboundEventChunker outboundEventChunker =
			new OutboundEventQueueWorker.OutboundEventChunker(OutboundEventQueueWorker.MAX_PAYLOAD_CHUNK_SIZE);
		final AssuranceEvent assuranceEvent = new AssuranceEvent(AssuranceConstants.AssuranceEventType.GENERIC,
				null);
	...
	public void test_chunk_eventPayloadSize5KB() {
		try {
			...
			final String expectedPayloadKeyValue = readPayloadFromResource("assurance_event_payload_key_value_5KB.txt");
			...
			final List<AssuranceEvent> chunkedEvents = outboundEventChunker.chunk(largeAssuranceEvent);
	...
	public void test_chunk_eventPayloadSize40KB() {
		try {
			...
			for (int i = 0; i < chunkedEvents.size(); i++) {
				...
				assertTrue(assuranceEvent.getJSONRepresentation().getBytes(StandardCharsets.UTF_8).length
						   < OutboundEventQueueWorker.MAX_EVENT_SIZE);

				final String chunkData = (String) assuranceEvent.getPayload().get(AssuranceConstants.AssuranceEventKeys.CHUNK_DATA);
				...
				assertEquals(i, metadata.get(AssuranceConstants.AssuranceEventKeys.CHUNK_SEQUENCE_NUMBER));
	...
	public void test_chunk_eventPayload_emptyLines() {
		try {
			...
			final String expectedPayloadKeyValue =
				readPayloadFromResource("assurance_large_event_payload_key_value_emptylines.txt");
