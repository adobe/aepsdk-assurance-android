...
import com.adobe.marketing.mobile.assurance.AssuranceClientInfo;
...
public class OutboundEventQueueWorkerTest {
	...
	public void setUp() throws Exception {
		...
		clientInfoEvent = new AssuranceEvent(AssuranceConstants.AssuranceEventType.CLIENT, Collections.EMPTY_MAP);
		when(mockAssuranceClientInfo.getData()).thenReturn(clientInfoData);

		outboundEventQueueWorker = new OutboundEventQueueWorker(mockExecutorService, mockAssuranceWebViewSocket,
				mockAssuranceClientInfo, queue, new OutboundEventQueueWorker.OutboundEventChunker(
	...
	public void test_prepare_sendsClientInfoEvent() {
		...
		when(mockAssuranceWebViewSocket.getState()).thenReturn(AssuranceWebViewSocket.SocketReadyState.OPEN);
	...
	public void test_canWork_when_socketDisconnected_workerUnblocked() {
		when(mockAssuranceWebViewSocket.getState()).thenReturn(AssuranceWebViewSocket.SocketReadyState.CLOSING);
	...
	public void test_canWork_socketDisconnected_workerBlocked() {
		when(mockAssuranceWebViewSocket.getState()).thenReturn(AssuranceWebViewSocket.SocketReadyState.CLOSED);
		outboundEventQueueWorker.block();
	...
	public void test_offer_workerNotYetStarted() {
		// Simulate the socket connection being open.
		Mockito.when(mockAssuranceWebViewSocket.getState()).thenReturn(AssuranceWebViewSocket.SocketReadyState.OPEN);
	...
	public void test_runnable_eventsSentWhenWorkIsQueued() {
		...
		// Should be submitted twice as the first invocation on start() is a no-op due to blocked queue.
		...
		assertEquals(event1.getJSONRepresentation(), new String(capturedEventData.get(1), Charset.forName("UTF-8")));
		assertEquals(event2.getJSONRepresentation(), new String(capturedEventData.get(2), Charset.forName("UTF-8")));
		assertEquals(event3.getJSONRepresentation(), new String(capturedEventData.get(3), Charset.forName("UTF-8")));
	}
	...
	public void test_runnable_eventsBlockedWhenCannotStartForwarding() {
		...
		// It is not feasible to match the client info event due to the random Event Id that is generated internally.
		// So, verify that the offered events do not match the event sent instead.
		assertNotEquals(event1.getJSONRepresentation(), new String(capturedEvent, Charset.forName("UTF-8")));
		assertNotEquals(event2.getJSONRepresentation(), new String(capturedEvent, Charset.forName("UTF-8")));
		assertNotEquals(event3.getJSONRepresentation(), new String(capturedEvent, Charset.forName("UTF-8")));
	...
	public void test_runnable_eventsBlockedWhenSocketNotConnected() {
		...
		Mockito.when(mockAssuranceWebViewSocket.getState()).thenReturn(AssuranceWebViewSocket.SocketReadyState.CLOSED);
		outboundEventQueueWorker.offer(event1);
		outboundEventQueueWorker.offer(event2);
		outboundEventQueueWorker.offer(event3);
	...
	public void test_sendEvent_payloadOverMaxPayloadSize_20KB() {
		...
		try {
			...
			final String expectedPayloadKeyValue = readPayloadFromResource("assurance_large_event_payload_key_value_20KB.txt");
			...
			final AssuranceEvent largeAssuranceEvent = new AssuranceEvent(AssuranceConstants.AssuranceEventType.GENERIC,
			...
			final String assuranceEventString = new String(capturedEventData.get(1), Charset.forName("UTF-8"));
			final AssuranceEvent actualEvent = new AssuranceEvent(assuranceEventString);
			assertEquals(largeAssuranceEvent.getJSONRepresentation(), actualEvent.getJSONRepresentation());
	...
	public void test_sendEvent_payloadOverMaxPayloadSize_40KB() {
		...
		try {
			// Prepare & read large payload from resources.
			final String expectedPayloadKeyValue = readPayloadFromResource("assurance_large_event_payload_key_value_40KB.txt");
			...
			for (int i = 1; i < capturedEventData.size(); i++) {
				...
				assertTrue(capturedChunkedEventBytes.length < OutboundEventQueueWorker.MAX_EVENT_SIZE);

				final String assuranceEventString = new String(capturedEventData.get(i), Charset.forName("UTF-8"));
				final AssuranceEvent actualEvent = new AssuranceEvent(assuranceEventString);
				final String chunkData = (String) actualEvent.getPayload().get(AssuranceConstants.AssuranceEventKeys.CHUNK_DATA);
				...
				assertEquals(i - 1, metadata.get(AssuranceConstants.AssuranceEventKeys.CHUNK_SEQUENCE_NUMBER));
	...
	public void test_sendEvent_payloadOverMaxPayloadSize_HTML() {
		...
		try {
			// Prepare & read large payload from resources.
			final String expectedPayloadKeyValue = readPayloadFromResource("assurance_large_event_payload_key_value_html.txt");
	...
	private String readPayloadFromResource(final String resourceName) throws IOException {
		...
		final BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(payloadValueStream));
