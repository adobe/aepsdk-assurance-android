...
public class AssuranceExtensionTest {
	...
	@Mock
	AssuranceSessionOrchestrator mockAssuranceSessionOrchestrator;
	...
	public void setup() {
		...
		Mockito.when(mockApplication.getSharedPreferences(AssuranceConstants.DataStoreKeys.DATASTORE_NAME,
					 Context.MODE_PRIVATE)).thenReturn(mockSharedPreference);
		...
		Whitebox.setInternalState(assuranceExtension, "assuranceStateManager", mockAssuranceStateManager);
		Whitebox.setInternalState(assuranceExtension, "assuranceConnectionDataStore", mockAssuranceConnectionDataStore);
		Whitebox.setInternalState(assuranceExtension, "assuranceSessionOrchestrator", mockAssuranceSessionOrchestrator);
		// This static variable needs to be reset everytime to ensure that the extension is started from the same state everytime.
	...
	public void test_AssuranceDoesNotShutDown_When_StartSessionAPICalled() throws Exception {
		...
		when(mockUri.getQueryParameter(START_URL_QUERY_KEY_SESSION_ID)).thenReturn(("6b55294e-32d4-49e8-9279-e3fe12a9d309"));

		assuranceExtension.startSession("griffon://?adb_validation_sessionid=6b55294e-32d4-49e8-9279-e3fe12a9d309");
		Thread.sleep(TimeUnit.SECONDS.toMillis(6L));

		// verify that the extension is still running
		verify(mockAssuranceSessionOrchestrator, never()).terminateSession();
		verify(mockAssuranceSessionOrchestrator, times(1)).createSession("6b55294e-32d4-49e8-9279-e3fe12a9d309",
	...
	public void test_StartSession_InvalidSessionID() throws Exception {
		...
		verify(mockAssuranceSessionOrchestrator, times(0)).createSession(anyString(),
				any(AssuranceConstants.AssuranceEnvironment.class), anyString());
	}
	...
	public void testProcessWildcardEvent() {
		...
		Event event = new Event.Builder("Mars landing event", EventType.ACQUISITION, EventSource.OS)
		.build();
		...
		verify(mockAssuranceSessionOrchestrator, times(1)).queueEvent(assuranceEventCaptor.capture());
		...
		assertEquals("Mars landing event", capturedEvent.payload.get(
						 AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_NAME));
		assertEquals(EventType.ACQUISITION.toLowerCase(),
					 capturedEvent.payload.get(AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_TYPE));
		assertEquals(EventSource.OS.toLowerCase(),
					 capturedEvent.payload.get(AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_SOURCE));
		assertEquals(event.getUniqueIdentifier(),
					 capturedEvent.payload.get(AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_UNIQUE_IDENTIFIER));
		assertEquals(event.getEventData(), capturedEvent.payload.get(
	...
	public void testProcessSharedStateEvent_RegularSharedState() throws Exception {
		...
		when(mockAssuranceSessionOrchestrator.getActiveSession()).thenReturn(mockSession);
		Mockito.doReturn(new SharedStateResult(SharedStateStatus.SET,
											   SAMPLE_STATE_DATA)).when(mockApi).getSharedState(anyString(),
													   nullable(Event.class), any(Boolean.class), any(SharedStateResolution.class));
		final ArgumentCaptor<AssuranceEvent> assuranceEventCaptor = ArgumentCaptor.forClass(AssuranceEvent.class);
		Event event = new Event.Builder(AssuranceConstants.SDKEventName.SHARED_STATE_CHANGE, EventType.HUB,
										EventSource.SHARED_STATE).setEventData(sharedStateChangeOwner)
		.build();
		...
		assertEquals("Shared state change", capturedEvent.payload.get(
						 AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_NAME));
		assertEquals(EventType.HUB.toLowerCase(),
					 capturedEvent.payload.get(AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_TYPE));
		assertEquals(EventSource.SHARED_STATE.toLowerCase(),
					 capturedEvent.payload.get(AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_SOURCE));
		assertEquals(event.getUniqueIdentifier(),
					 capturedEvent.payload.get(AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_UNIQUE_IDENTIFIER));
		assertEquals(sharedStateChangeOwner,
					 capturedEvent.payload.get(AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_DATA));

		// unpack and verify shared state content in assurance event
		HashMap <String, Object> payloadMetadata = (HashMap <String, Object>) capturedEvent.payload.get(
					AssuranceTestConstants.PayloadDataKeys.METADATA);
		HashMap <String, Object> sharedStateContent = (HashMap <String, Object>) payloadMetadata.get(
					AssuranceTestConstants.PayloadDataKeys.STATE_CONTENTS);
	...
	public void testProcessSharedStateEvent_XDMSharedState() throws Exception {
		...
		sharedStateChangeOwner.put("stateowner", "superextension");
		when(mockAssuranceSessionOrchestrator.getActiveSession()).thenReturn(mockSession);
		doReturn(new SharedStateResult(SharedStateStatus.SET, SAMPLE_STATE_DATA)).when(mockApi).getXDMSharedState(anyString(),
				nullable(Event.class), any(Boolean.class), any(SharedStateResolution.class));
		final ArgumentCaptor<AssuranceEvent> assuranceEventCaptor = ArgumentCaptor.forClass(AssuranceEvent.class);
		Event event = new Event.Builder(AssuranceConstants.SDKEventName.XDM_SHARED_STATE_CHANGE, EventType.HUB,
										EventSource.SHARED_STATE).setEventData(sharedStateChangeOwner)
		.build();
		...
		// Verify the shared state content event payload
		assertEquals("Shared state change (XDM)", capturedEvent.payload.get(
						 AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_NAME));
		assertEquals(EventType.HUB.toLowerCase(),
					 capturedEvent.payload.get(AssuranceTestConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_TYPE));
		assertEquals(EventSource.SHARED_STATE.toLowerCase(),
		...
		HashMap <String, Object> payloadMetadata = (HashMap <String, Object>) capturedEvent.payload.get(
					AssuranceTestConstants.PayloadDataKeys.METADATA);
		HashMap <String, Object> sharedStateContent = (HashMap <String, Object>) payloadMetadata.get(
					AssuranceTestConstants.PayloadDataKeys.XDM_STATE_DATA);
		assertEquals(SAMPLE_STATE_DATA, sharedStateContent);
	}
	...
	public void testProcessSharedStateEvent_WhenInvalidSharedStateEvent() {
		...
		when(mockAssuranceSessionOrchestrator.getActiveSession()).thenReturn(null);
		Event event = new Event.Builder("Shared State Event", EventType.HUB,
										EventSource.SHARED_STATE).setEventData(invalidStateChangeOwnerData)
	...
	public void testProcessSharedStateEvent_invalidSharedStateEvent_noOwner() {
		// setup
		Event event = new Event.Builder("Shared State Event", EventType.HUB, EventSource.SHARED_STATE).setEventData(null)
		.build(); // no shared state change owner is provided
	...
	public void test_logLocalUI() {
		...
		// verify
		verify(mockSession, times(1)).logLocalUI(AssuranceConstants.UILogColorVisibility.HIGH, "dummy");
	}
