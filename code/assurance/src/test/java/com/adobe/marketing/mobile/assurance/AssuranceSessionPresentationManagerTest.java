...
public class AssuranceSessionPresentationManagerTest extends TestCase {
	...
	@Mock
	private AssuranceFloatingButton mockAssuranceFloatingButton;

	@Mock
	private AssuranceConnectionStatusUI mockConnectionStatusUI;

	@Mock
	private AssurancePinCodeEntryURLProvider mockPinCodeEntryURLProvider;
	...
	public void setUp() throws Exception {
		...
		assuranceSessionPresentationManager = new AssuranceSessionPresentationManager(mockAssuranceStateManager,
				mockSessionUIOperationHandler, mockApplicationHandle);
		Whitebox.setInternalState(assuranceSessionPresentationManager, "button", mockAssuranceFloatingButton);
		Whitebox.setInternalState(assuranceSessionPresentationManager, "statusUI", mockConnectionStatusUI);
		Whitebox.setInternalState(assuranceSessionPresentationManager, "urlProvider", mockPinCodeEntryURLProvider);
	}

	@Test
	public void testLogLocalUI_AddsUILog() {
		assuranceSessionPresentationManager.logLocalUI(AssuranceConstants.UILogColorVisibility.HIGH,
				"Sample StatusUI Message");

		verify(mockConnectionStatusUI).
		addUILog(eq(AssuranceConstants.UILogColorVisibility.HIGH), eq("Sample StatusUI Message"));
	}
	...
	public void testOnSessionConnected_NotifiesPINDialog_ShowsFloatingButton_LogsUI() {
		...
		verify(mockAssuranceFloatingButton).setCurrentGraphic(AssuranceFloatingButtonView.Graphic.CONNECTED);
		verify(mockAssuranceFloatingButton).display();
		verify(mockConnectionStatusUI).addUILog(eq(AssuranceConstants.UILogColorVisibility.LOW),
	...
	public void testOnSessionDisconnected_CONNECTION_LIMIT() {
		...
		assuranceSessionPresentationManager.onSessionDisconnected(AssuranceConstants.SocketCloseCode.CONNECTION_LIMIT);
	...
	public void testOnSessionDisconnected_ABNORMAL() {
		...
		assuranceSessionPresentationManager.onSessionDisconnected(AssuranceConstants.SocketCloseCode.ABNORMAL);
	...
	public void testOnSessionReconnecting() {
		...
		verify(mockAssuranceFloatingButton).setCurrentGraphic(AssuranceFloatingButtonView.Graphic.DISCONNECTED);
		verify(mockAssuranceFloatingButton).display();
		verify(mockConnectionStatusUI).addUILog(eq(AssuranceConstants.UILogColorVisibility.HIGH),
