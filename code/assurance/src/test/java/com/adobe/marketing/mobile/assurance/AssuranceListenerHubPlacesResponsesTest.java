...
import com.adobe.marketing.mobile.assurance.AssuranceListenerHubPlacesResponses;
...
public class AssuranceListenerHubPlacesResponsesTest {
	...
	public void testHear_when_RegionResponseEvent() {
		...
		// verify
		verify(mockAssuranceExtension, times(1)).logLocalUI(AssuranceConstants.UILogColorVisibility.HIGH,
	...
	public void testHear_when_InvalidEvent() {
		Event event = new Event.Builder(null, EventType.PLACES,
	...
	private Event REGION_EVENT() {
		...
		Event event = new Event.Builder(REGION_EVENT_EVENT_NAME, EventType.PLACES,
	...
	private Event NEAR_BY_PLACES_EVENT() {
		...
		Event event = new Event.Builder(NEARBY_PLACES_EVENT_NAME, EventType.PLACES,
										EventSource.RESPONSE_CONTENT).setEventData(eventData).build();
