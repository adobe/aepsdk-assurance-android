...
public class AssuranceStateManagerTest extends TestCase {
	...
	public void setUp() throws Exception {
		...
		when(mockApplication.getSharedPreferences(AssuranceConstants.DataStoreKeys.DATASTORE_NAME,
	...
	public void test_ShareAssuranceState() {
		...
		// verify
		verify(mockApi, times(1)).createSharedState(sharedStateCaptor.capture(),
				lastEventCaptor.capture());
		assertEquals("SampleSessionId", sharedStateCaptor.getValue().get(
						 AssuranceTestConstants.SharedStateKeys.ASSURANCE_STATE_SESSION_ID));
		assertNotNull(sharedStateCaptor.getValue().get(AssuranceTestConstants.SharedStateKeys.ASSURANCE_STATE_CLIENT_ID));
		assertNotNull(sharedStateCaptor.getValue().get(AssuranceTestConstants.SharedStateKeys.ASSURANCE_STATE_INTEGRATION_ID));
	...
	public void test_shareAssuranceSharedState_emptySessionID() {
		...
		verify(mockApi, times(1)).createSharedState(sharedStateCaptor.capture(),
				lastEventCaptor.capture());
		assertNull(sharedStateCaptor.getValue().get(
					   AssuranceTestConstants.SharedStateKeys.ASSURANCE_STATE_SESSION_ID));
		assertNull(sharedStateCaptor.getValue().get(
					   AssuranceTestConstants.SharedStateKeys.ASSURANCE_STATE_INTEGRATION_ID));
	}
	...
	public void test_clearAssuranceState() {
		...
		// verify
		verify(mockApi, times(1)).createSharedState(sharedStateCaptor.capture(), lastEventCaptor.capture());
	...
	public void test_GetAllExtensionStateData() throws Exception {
		...
		doAnswer(new Answer<SharedStateResult>() {
			@Override
			public SharedStateResult answer(InvocationOnMock invocation) throws Throwable {
				...
				if ("com.adobe.module.places".equals(extension)) {
					...
					return new SharedStateResult(SharedStateStatus.SET, SAMPLE_STATE_DATA);
				}  else if ("com.adobe.module.eventhub".equals(extension)) {
					return new SharedStateResult(SharedStateStatus.SET, extensionDetails);
				} else {
					return null;
				}
			}
		}).when(mockApi).getSharedState(any(String.class), nullable(Event.class), any(Boolean.class),
		...
		doAnswer(new Answer<SharedStateResult>() {
			...
			public SharedStateResult answer(InvocationOnMock invocation) throws Throwable {
				...
				if ("com.adobe.edge.consent".equals(extension)) {
					return new SharedStateResult(SharedStateStatus.SET, SAMPLE_XDM_STATE_DATA);
				} else {
					return null;
				}
			}
		}).when(mockApi).getXDMSharedState(any(String.class), nullable(Event.class), any(Boolean.class),
										   any(SharedStateResolution.class));
		...
		assertEquals("EventHub State", eventHubStateEvent.payload.get(
						 AssuranceConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_NAME));
		assertEquals(EventType.HUB, eventHubStateEvent.payload.get(
						 AssuranceConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_TYPE));
		assertEquals(EventSource.SHARED_STATE,
					 eventHubStateEvent.payload.get(AssuranceConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_SOURCE));
		assertEquals("com.adobe.module.eventhub", ((Map) eventHubStateEvent.payload.get(
						 AssuranceConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_DATA)).get("stateowner"));
		assertEquals(extensionDetails,
					 ((Map) eventHubStateEvent.payload.get(AssuranceConstants.PayloadDataKeys.METADATA)).get("state.data"));
		...
		assertEquals("com.adobe.module.configuration", ((Map) configurationStateEvent.payload.get(
						 AssuranceConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_DATA)).get("stateowner"));
		assertEquals(SAMPLE_STATE_DATA,
					 ((Map) configurationStateEvent.payload.get(AssuranceConstants.PayloadDataKeys.METADATA)).get("state.data"));


		// verify state event 3  : Consent with XDM shared state
		final AssuranceEvent consentStateEvent = stateEvents.get(2);
		assertEquals("com.adobe.edge.consent", ((Map) consentStateEvent.payload.get(
				AssuranceConstants.GenericEventPayloadKey.ACP_EXTENSION_EVENT_DATA)).get("stateowner"));
		assertEquals(SAMPLE_XDM_STATE_DATA,
					 ((Map) consentStateEvent.payload.get(AssuranceConstants.PayloadDataKeys.METADATA)).get("xdm.state.data"));
	}
	...
	public void test_getAllExtensionStateData_noRegisteredExtensions() throws Exception {
		...
		when(mockApi.getSharedState(eq(AssuranceConstants.SDKSharedStateName.EVENTHUB),
									any(Event.class), any(Boolean.class), any(SharedStateResolution.class))).thenReturn(res);
	...
	private Event sampleEvent() {
		return new Event.Builder("Mars landing event", EventType.ACQUISITION, EventSource.OS).build();
	}
