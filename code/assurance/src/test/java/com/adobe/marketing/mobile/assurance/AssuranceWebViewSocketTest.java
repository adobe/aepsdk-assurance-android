...
public class AssuranceWebViewSocketTest {
	...
	private static final String CONST_URL = "wss://connect.griffon.adobe.com/client/v1?" +
	...
	@Mock
	AssuranceWebViewSocketHandler mockAssuranceWebViewSocketHandler;

	@Mock
	WebView mockWebview;

	@Mock
	ExecutorService webViewExecutor;
	...
	public void test_connect() throws InterruptedException {
		...
		assertEquals(AssuranceWebViewSocket.SocketReadyState.CONNECTING, assuranceWebViewSocket.getState());

		// verify onSocketStateChange is called
		verify(mockAssuranceWebViewSocketHandler).onSocketStateChange(assuranceWebViewSocket,
	...
	public void test_connect_invalidURL() throws InterruptedException {
		...
		assertNotEquals(AssuranceWebViewSocket.SocketReadyState.CONNECTING, assuranceWebViewSocket.getState());

		// verify onSocketStateChange is not called
		verify(mockAssuranceWebViewSocketHandler, never()).onSocketStateChange(assuranceWebViewSocket,
				AssuranceWebViewSocket.SocketReadyState.CONNECTING);
	...
	public void test_disconnect() {
		...
		// verify state is updated
		assertEquals(AssuranceWebViewSocket.SocketReadyState.CLOSING, assuranceWebViewSocket.getState());
	...
	public void test_sendData() throws InterruptedException {
		...
		PowerMockito.when(Base64.encodeToString(any(byte[].class), anyInt())).thenReturn(encodedDataString);
		...
		assertEquals(Integer.valueOf(Base64.NO_WRAP | Base64.NO_PADDING), argumentCaptor1.getValue());

		// verify webview load url is called
		ArgumentCaptor<String> argumentCaptor2 = ArgumentCaptor.forClass(String.class);
		verify(mockWebview).loadUrl(argumentCaptor2.capture());
		assertEquals("javascript: sendData('" + encodedDataString + "')", argumentCaptor2.getValue());
	}
	...
	private void mockExecutorService() {
		...
		}).when(webViewExecutor).submit(any(Runnable.class));
	}
	...
	private void mockMainHandlerAndRunTheRunnable() {
		...
		}).when(mainHandlerMock).post(any(Runnable.class));
	}
	...
	private void mockValidURL() {
		...
		when(mockUri.getQueryParameter("sessionId")).thenReturn(("d600bba7-f90e-45a9-8022-78edda3edda5"));
		when(mockUri.getQueryParameter("sessionId")).thenReturn(("d600bba7-f90e-45a9-8022-78edda3edda5"));
		...
		when(mockUri.getQueryParameter("clientId")).thenReturn(("C8385D85-9CE3-409E-92C2-565E7E59D69C"));
	...
	private void mockValidInvalidURI() {
		...
		when(mockUri.getQueryParameter("sessionId")).thenReturn(("RANDOM_d600bba7-f90e-45a9-8022-78edda3edda5"));
		when(mockUri.getQueryParameter("sessionId")).thenReturn(("d600bba7-f90e-45a9-8022-78edda3edda5"));
		when(mockUri.getQueryParameter("token")).thenReturn(("1234"));
		when(mockUri.getQueryParameter("orgId")).thenReturn("972C898555E9F7BC7F000101@AdobeOrg");
		when(mockUri.getQueryParameter("clientId")).thenReturn(("C8385D85-9CE3-409E-92C2-565E7E59D69C"));
